...
st.title("Welcome to Capstone Project on human activity recognition with smartphones")
...
st.subheader("\n\nNumber of Columns & Rows in Dataset")
st.write(df.shape)

st.subheader("Describing the Dataset")
st.write(df.describe())

st.subheader("Number of Duplicates in Dataset")
st.write(len(df[df.duplicated()]))

st.subheader("Unique Acitivities With Value Count")
...
st.subheader("Finding the corellation Matrix between column and removing columns from dataset having corelation  >0.95")
st.write("\nCleaned DataFrame:\n", df_cleaned.shape)
...
st.subheader('SVC Linear: Model accuracy score with default hyperparameters: {0:0.4f}'. format(accuracy_score(y_test, y_pred)))
...
# compute and print accuracy score
st.subheader('SVC poly: Model accuracy score with default hyperparameters: {0:0.4f}'. format(accuracy_score(y_test, y_pred)))
st.write("\nSVC poly Classification Report:\n\n", classification_report(y_test, y_pred))

cm = confusion_matrix(y_test, y_pred)

st.write('\n\nSVC poly Confusion matrix\n\n', cm)
...
# compute and print accuracy score
st.subheader('SVC RBF: Model accuracy score with default hyperparameters: {0:0.4f}'. format(accuracy_score(y_test, y_pred)))

st.write("\nSVC RBF Classification Report:\n\n", classification_report(y_test, y_pred))

cm = confusion_matrix(y_test, y_pred)

st.write('\n\nSVC RBF Confusion matrix\n\n', cm)


# In[41]:

st.header("Now Loading the Test Data")
df_test=pd.read_csv(path+"/test.csv")

st.subheader("Top 5 Row the Test Data")
...
ytest_pred= svc_linear.predict(Xtest)

# compute and print accuracy score
st.write("\nTest Data SVC Linear Classification Report:\n\n", classification_report(ytest, ytest_pred))


cm = confusion_matrix(ytest, ytest_pred)

st.write('\n\nTest Data SVC Linear Confusion matrix\n\n', cm)
...
st.header("Preparing a Model using ANN ")
...
st.subheader(f"ANN Model Test Accuracy: {test_acc:.4f}")
...
st.write("ANN Model Predicted Classes:", encoder.inverse_transform(predicted_classes[:10]))
st.write("Actual Classes:", encoder.inverse_transform(y_test[:10]))
...
st.write("ANN Model Predicted Classes for Test Data:", encoder.inverse_transform(predicted_classes_on_testData[:10]))
st.write("Actual Classes for Test Data:", ytest[:10])
